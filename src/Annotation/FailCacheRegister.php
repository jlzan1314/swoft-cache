 ...
 * @since 2.0
 */
class FailCacheRegister
{
	private static $data = [];

	public static function register(string $class, string $method, FailCache $annotation)
	{
	...
	public static function getAnnotation(string $class, string $method): ?FailCache
	{
		return self::$data[$class][$method];
	}

	public static function getFailCacheValue(string $className, string $method, array $arguments): array
	{
		$annotation = self::getAnnotation($className, $method);

		$prefix = $annotation->getPrefix() ?? ($className . '::' . $method);
